...
def main():
    ...
    for t in np.arange(0, T + dt/2, dt):
        ...
        print("[*] t=%.2f: pos=[%.2f, %.2f, %.2f], vel=[%.2f, %.2f, %.2f], acc=[%.2f, %.2f, %.2f]", t, p[0], p[1], p[2], v[0], v[1], v[2], a[0], a[1], a[2])
        ...
        print(pos_leader)
        maneuver_l.waypoint({
              'x': p[0], 'y': p[1], 'z': p[2], 'yaw': 0, 'duration': 0,
              'vx': v[0], 'vy': v[1], 'vz': v[2], 'wz': 0, 'ax': a[0], 'ay': a[1], 'az': a[2]
            })
    ...
    maneuver_l.set_velocity_limit(vmax, 1)
    maneuver_l.set_acceleration_limit(amax, 1)
    ...
    wasnt_in_fov = True # flag to signal when the leader exits and enters in the FOV of the camera

    # Statistiche
    in_fov_count = 0
    out_fov_count = 0
    pom_l.log_state('/home/giuliano-livi/tk3lab-ws/src/final_project/pom.log')
    try:
        while loop_count < max_loops:
            ...
            if is_in_fov:
                ...
                leader_vel, wasnt_in_fov = tracker.velocity_estimator.update(pos_leader, wasnt_in_fov)
                wasnt_in_fov = False
